    ...
    return next.handle(request).pipe(
      catchError(error => {
        if (error) {
          switch (error.status) {
            case 400:
              if (error.error.errors) { //check if error.error contains an object
                const errorsObj = error.error.errors
                const modalStateErrors = [];
                for (const key in errorsObj) {
                  if (errorsObj[key]) {
                    modalStateErrors.push(errorsObj[key])
                  }
                }

                throw modalStateErrors.flat(); //flat is a new method form es2019 to concat subarrays into strings
              }
              else {
